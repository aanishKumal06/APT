            ...
            <c:forEach var="room" items="${rooms}">
                ...
                <tr>
				    ...
				    <c:if test="${room.roomStatus != 'occupied'}">
				        ...
				        <!-- Success Message -->
				        <div id="successMessage-${room.roomId}" class="dialog-overlay" style="display: none;">
				            <div class="dialog-box">
				                <p>${message}</p>
				                <button onclick="closeSuccessMessage('${room.roomId}')">Close</button>
				            </div>
				        </div>
				    </c:if>
				</td>
					...
					function submitDelete(roomId) {
					    // Submit the form
					    document.getElementById('deleteForm-' + roomId)
					    // Optionally show success message later via server-side redirect or response
					}
					
					function closeSuccessMessage(roomId) {
					    document.getElementById('successMessage-' + roomId).style.display = 'none';
					}
					</script>
